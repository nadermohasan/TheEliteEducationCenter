...
import {
  ...
  Save, Power, PowerOff, Settings, Info, List, Eye, EyeOff, RotateCcw, Zap
} from "lucide-react";
...
export default function TeacherDashboard() {
  ...
  return (
    <div className="teacher-container">
      ...
      <main className="teacher-main">
        ...
        <div className="stats-grid">
          <div className="stat-card">
            ...
            <div className="stat-content"><span className="stat-label">إجمالي الأسئلة</span><span className="stat-number">{stats.totalQuestions}</span></div>
          </div>
          <div className="stat-card">
            ...
            <div className="form-section">
              ...
              <div className="form-grid">
                ...
                <div className="form-group">
                  ...
                  <input type="number" className="modern-input" value={formData.degree} onChange={(e) => setFormData({ ...formData, degree: e.target.value })} placeholder="1" min="1" max="100" />
                </div>
        ...
        <section className="table-card">
          <div className="card-header">
            <h2 className="card-title"><BookOpen size={20} className="icon-accent" /> الأسئلة المتوفرة في البنك</h2>
            <div className="card-header-actions">
              <span className="badge-count">{filteredQuestions.length} سؤال</span>
              <div className="search-wrapper">
                <Search size={18} className="search-icon" />
                <input type="text" placeholder="ابحث عن سؤال..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="search-input" />
              </div>
            </div>
          </div>
          ...
          <div className="table-responsive">
            {loading && questions.length === 0 ? (
                  ...
                  {filteredQuestions.map((q) => {
                    ...
                    return (
                      <tr key={q.id} className={q.is_active ? '' : 'inactive-row'}>
                        <td><span className="badge bg-slate">{q.subjects?.name || "غير محدد"}</span></td>
                        <td><span className="badge bg-light">{q.branch || "عام"}</span></td>
                        <td className="text-center font-bold">{q.unit_number || "-"}</td>
                        ...
                        <td className="text-center font-bold">{q.degree || 1}</td>
                        <td className="text-center"><span className="correct-badge">{labels[q.correct_option]}</span></td>
                        <td>
                          <div className="action-buttons">
                            <button className="btn-icon edit" onClick={() => loadQuestionForEdit(q)} title="تعديل"><Edit2 size={16} /></button>
                            <button className="btn-icon delete" onClick={() => deleteQuestion(q.id)} title="حذف"><Trash2 size={16} /></button>
                          </div>
                        </td>
                      </tr>
                    );
              ...
              <div className="empty-state">
                ...
                <p>{searchTerm ? "لم نعثر على نتائج مطابقة لبحثك. جرب كلمات أخرى." : "بنك الأسئلة فارغ. ابدأ بإضافة سؤالك الأول باستخدام النموذج أعلاه."}</p>
              </div>
            )}
          </div>
        </section>
        ...
        <section className="table-card">
          <div className="card-header">
            <h2 className="card-title"><Upload size={20} className="icon-accent" /> سجل الدفعات المرفوعة</h2>
          </div>
          <div className="table-responsive">
                      ...
                      <td className="text-center">
                        <div className="action-buttons">
                          <button
                            ...
                            title={batch.isActive ? "تعطيل" : "تفعيل"}
                          >
                            {batch.isActive ? <PowerOff size={16} /> : <Power size={16} />}
                          </button>
                          <button className="btn-icon delete" onClick={() => deleteBatchQuestions(batch.id, idx)} title="حذف">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      </td>
      ...
      {/* --- Modals --- */}
     {/* Passage Modal – UX Enhanced (with search) */}
{/* Passage Modal – Clean & Safe */}
{showPassageModal && (
  ...
  .map(s => <option key={s.id} value={s.id}>{s.name}</option>)
            ...
            <div className="form-group full-width" style={{ marginBottom: '16px' }}>
              ...
              <textarea
                ...
                placeholder="أدخل النص الكامل للقطعة..."
                style={{ minHeight: '120px', lineHeight: '1.8' }}
              />
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
              {editingPassage ? (
                <>
                  <button type="button" className="btn-secondary" onClick={() => { setEditingPassage(null); setPassageForm({ title: "", passage_text: "", subject_id: "" }); }}>
                    إلغاء
                  </button>
                ...
                <>
                  <button type="button" className="btn-secondary" onClick={() => { setShowPassageModal(false); setPassageForm({ title: "", passage_text: "", subject_id: "" }); }}>
                    إلغاء
                  </button>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          <h4 style={{ margin: 0, fontSize: '1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            النصوص الحالية
            <span style={{
              background: '#fef3c7', color: '#92400e', padding: '2px 10px', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 700
            }}>{passages.length}</span>
          </h4>
                  ...
                  >
                    <div>
                      ...
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        <span style={{
                          background: '#f1f5f9', color: '#334155', padding: '2px 10px', borderRadius: '12px',
                          fontSize: '0.75rem', fontWeight: 600
                        }}>
                          {subject?.name || 'غير محدد'}
                        </span>
                        <span style={{ fontSize: '0.75rem', color: '#94a3b8' }}>
                          {p.passage_text.length > 60 ? p.passage_text.substring(0, 60) + '...' : p.passage_text}
                        </span>
                      </div>
                    </div>
                    <div className="action-buttons" style={{ gap: '4px' }}>
                      <button
                        ...
                        onClick={() => {
                          setEditingPassage(p);
                          setPassageForm({ title: p.title, passage_text: p.passage_text, subject_id: p.subject_id.toString() });
                        }}
                        title="تعديل"
      ...
      {/* Bulk Modal - تحسين الأزرار والتمرير */}
      {showBulkModal && (
     ...
     {/* Settings Modal – Simplified Creative UX */}
{showSettingsModal && (
  <div className="modal-backdrop" onClick={() => setShowSettingsModal(false)}>
    <div className="modal-container settings-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '850px' }}>
      <div className="modal-header" style={{ background: 'linear-gradient(135deg, #f0f4ff, #ffffff)' }}>
        <h3><Settings size={20} className="icon-accent" /> إعدادات المواد</h3>
      ...
      <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto', padding: '20px' }}>
        {/* قسم التطبيق الجماعي */}
        <div style={{
          background: '#ffffff', borderRadius: '16px', padding: '16px 20px', marginBottom: '24px',
          border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(0,0,0,0.03)'
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            <span style={{ fontWeight: 700, fontSize: '0.95rem', whiteSpace: 'nowrap' }}>⚡ تطبيق سريع:</span>
            <input
              type="number"
              min="1"
              max="180"
            ...
            <button
              ...
              onClick={() => {
                ...
                if (isNaN(dur) || isNaN(que)) {
                  toast.error('أدخل قيماً صحيحة');
                  return;
                }
                const newDurations = {}, newCounts = {};
                subjects.forEach(s => {
                  newDurations[s.id] = dur;
                  newCounts[s.id] = que;
                });
                setEditingDurations(newDurations);
                setEditingQuestionsCount(newCounts);
                toast.success(`تم تعيين المدة ${dur} دقيقة و ${que} سؤال لجميع المواد`);
              }}
            >
              <CheckCircle2 size={16} /> تطبيق على الكل
            </button>
            <button
              ...
              onClick={async () => {
                const confirmed = await showConfirm({
                  title: 'إعادة التعيين',
                  message: 'سيتم إعادة جميع المواد إلى 60 دقيقة و 40 سؤال.',
                  confirmText: 'إعادة',
                  cancelText: 'إلغاء'
                });
                if (!confirmed) return;
                ...
                subjects.forEach(s => {
                  defaultsDurations[s.id] = 60;
                  defaultsCounts[s.id] = 40;
                });
                setEditingDurations(defaultsDurations);
                setEditingQuestionsCount(defaultsCounts);
                toast.success('تمت إعادة جميع المواد للإعدادات الافتراضية');
              }}
            >
              ...
              >
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                      ...
                      <span style={{ fontWeight: 700, fontSize: '1rem', color: '#0f172a' }}>{subject.name}</span>
                    </div>
                    <span style={{ fontSize: '0.8rem', color: '#64748b', marginRight: '26px' }}>
                      {subject.branch ? `الفرع: ${subject.branch}` : 'عام'}
                    </span>
                  </div>
                  <button
                    className="btn-text"
                    style={{ fontSize: '0.75rem', padding: '4px 8px', color: '#64748b' }}
                    onClick={() => {
                      setEditingDurations(prev => ({ ...prev, [subject.id]: 60 }));
                      setEditingQuestionsCount(prev => ({ ...prev, [subject.id]: 40 }));
                      toast.success(`تم إعادة ${subject.name} إلى الافتراضي`);
                    }}
                  >
                    <RotateCcw size={14} style={{ marginLeft: '4px' }} /> افتراضي
                  </button>
                </div>
                ...
                <div style={{ marginBottom: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>⏱️ المدة (دقيقة)</span>
                    <span style={{ fontSize: '0.85rem', fontWeight: 800, color: '#2563eb' }}>{duration}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <button
                      onClick={() => handleSubjectDurationChange(subject.id, Math.max(1, duration - 1))}
                      disabled={duration <= 1}
                      style={{
                    ...
                    <input
                      ...
                      max="180"
                      className="modern-input"
                      style={{ flex: 1, textAlign: 'center', fontSize: '0.95rem', fontWeight: 600 }}
                    />
                    <button
                      onClick={() => handleSubjectDurationChange(subject.id, Math.min(180, duration + 1))}
                ...
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                    <span style={{ fontSize: '0.85rem', fontWeight: 600 }}>📝 عدد الأسئلة</span>
                    <span style={{ fontSize: '0.85rem', fontWeight: 800, color: '#7c3aed' }}>{questions}</span>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <button
                      onClick={() => handleSubjectQuestionsCountChange(subject.id, Math.max(1, questions - 1))}
                    ...
                    />
                    <button
                      onClick={() => handleSubjectQuestionsCountChange(subject.id, Math.min(100, questions + 1))}
      ...
      <div className="modal-footer">
        <button className="btn-secondary" onClick={() => setShowSettingsModal(false)}>إلغاء</button>
        <button className="btn-primary" onClick={handleSaveAllSettings} disabled={savingSettings}>
          <Save size={18} /> {savingSettings ? 'جاري الحفظ...' : 'حفظ جميع الإعدادات'}
        </button>
      </div>
    </div>
  </div>
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; }
        ...
        .modern-table th { background: #f8fafc; padding: 16px 24px; color: var(--c-text-muted); font-weight: 700; font-size: 0.9rem; border-bottom: 2px solid var(--c-border); text-transform: uppercase; letter-spacing: 0.05em; }
        .modern-table td { padding: 16px 24px; border-bottom: 1px solid var(--c-secondary); color: var(--c-text-body); vertical-align: middle; transition: background 0.2s; font-size: 0.95rem; }
        ...
        .active-badge { background: var(--c-success-light); color: #16a34a; border: 1px solid #bbf7d0; }
        ...
        .card-header-actions { display: flex; align-items: center; gap: 20px; }
